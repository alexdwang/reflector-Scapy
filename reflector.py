...
def main(argv):
   ...
   def PacketHandler3(pkt):
	 ...
	 if pkt[ARP].pdst==vict_ip:
	    ...
	    p.show()
	    sendp(p,iface=interface)
#      if pkt[0][1].pdst==vict_ip:
#	 arp=ARP(pdst=pkt[0][1].psrc,psrc=vict_ip,hwdst=pkt[0][1].hwsrc,hwsrc=vict_eth,op=2)
#	 eth=Ether(dst=pkt[0][1].hwsrc,src=vict_eth)
#	 p=eth/arp
#	 sendp(p,iface=interface)
 #     elif pkt[0][1].pdst==refl_ip:
#	 eth=Ether(dst=pkt[0][1].hwsrc,src=refl_eth)
#	 arp=ARP(pdst=pkt[0][1].psrc,psrc=refl_ip,hwdst=pkt[0][1].hwsrc,hwsrc=refl_eth,op=2)
#	 p=eth/arp
#	 sendp(p,iface=interface)

   def vic_ip_s(para):
      dpkt = sniff(iface = interface, filter="(tcp or udp or ip) and (dst "+vict_ip+")", prn = PacketHandler)
   def src_res(para):
   ...
   thread.start_new_thread(vic_ip_s,("",))   
   thread.start_new_thread(src_res,("",)) 
   thread.start_new_thread(arp_vic,("",))
